...
describe("poseTracking: Phase1統合（左右補正・トラッカー・平滑化の連携）", () => {
  ...
  it("result.trackerStatsに、検出できなかったフレームぶんのcoastingが反映される", async () => {
    useDetections([normalizedPose(0.2, 0.4)], [], [normalizedPose(0.21, 0.4)]);

    const result = await analyzeTrackedMotion(videoStub({ duration: 0.2 }), 10, undefined, null, {
      smoothing: { enabled: false },
    });

    expect(result.trackerStats).toBeDefined();
    expect(result.trackerStats!.updateCount).toBe(3);
    expect(result.trackerStats!.coastingFrameCount).toBeGreaterThan(0);
    expect(result.trackerStats!.matchedFrameCount).toBeGreaterThan(0);
  });
});
