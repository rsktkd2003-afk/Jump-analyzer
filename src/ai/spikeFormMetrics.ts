// =============================================================
// スパイクフォーム評価: 51指標の定義と測定処理。
// spikeFormEvaluation.ts から分離した、個別指標のmeasure/score/confidence/
// description生成を専門に担当するモジュール。
// カテゴリ集計・overallScore・landing除外・aerialPostureScore/星評価・
// 最終結果の組み立てはspikeFormEvaluation.ts側の責務のままここには含めない。
// =============================================================

import type { TrackedFrame, TrackedLandmark } from "./trackingAnalyzer";
import { findEnginePhase, type EnginePhaseName, type JumpPhaseEngineResult } from "./jumpPhaseEngine";
import {
  calculateAngle,
...
import {
  ...
  WOBBLE_SCORE_BY_REVERSAL_COUNT,
  WOBBLE_SMOOTHING_WINDOW,
} from "./aerialPostureScoring";
import type { EvaluationCategoryId, EvaluationMetric, SpikeArmForm } from "./spikeFormEvaluationTypes";
...
/**
 * 51指標すべてを測定し、score・confidence・descriptionまで生成したEvaluationMetric[]を返す。
 * frames/engineからEvaluationContext（bodyScale/fpsを含む）を組み立てる責務もここに持つ。
 * カテゴリ集計・overallScore・landing除外・最終結果の組み立てはspikeFormEvaluation.ts側で行う。
 */
export function evaluateSpikeFormMetrics(
  frames: TrackedFrame[],
  engine: JumpPhaseEngineResult,
  ...
  return metricDefinitions.map((definition): EvaluationMetric => {
